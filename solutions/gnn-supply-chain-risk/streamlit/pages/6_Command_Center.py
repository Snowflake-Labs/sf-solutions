...
st.set_page_config(page_title="Command Center", page_icon=None, layout="wide", initial_sidebar_state="expanded")
            ...
            alerts.append(
                {
                    ...
                    "content": (
                        f"Risk score: {row['RISK_SCORE']:.0%}. "
                        f"Located in {row['COUNTRY_CODE']}. "
            ...
            alerts.append(
                {
                    ...
                    "content": (
                        f"{row['DEPENDENT_COUNT']} vendors dependent. "
                        f"Impact: {row['IMPACT_SCORE']:.0%}. Status: {status}"
                    ),
...
def load_alert_summary(_session):
    ...
    queries = {
        "critical_count": (
            f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.RISK_SCORES"
            f" WHERE RISK_CATEGORY = 'CRITICAL' AND NODE_TYPE = 'SUPPLIER'"
        ),
        "high_count": (
            f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.RISK_SCORES"
            f" WHERE RISK_CATEGORY = 'HIGH' AND NODE_TYPE = 'SUPPLIER'"
        ),
        "medium_count": (
            f"SELECT COUNT(*) as CNT FROM {DB_SCHEMA}.RISK_SCORES"
            f" WHERE RISK_CATEGORY = 'MEDIUM' AND NODE_TYPE = 'SUPPLIER'"
        ),
...
def render_risk_trend_chart(height=200):
    ...
    fig.update_layout(
        ...
        xaxis=dict(showgrid=False, tickfont=dict(color="#64748b", size=10), tickformat="%b %d"),
        yaxis=dict(
            showgrid=True, gridcolor="rgba(51, 65, 85, 0.3)", tickfont=dict(color="#64748b", size=10), title=None
        ),
    )
...
def main():
    ...
    with col1:
        ...
        with filter_col1:
            severity_filter = st.selectbox("Filter by severity:", ["All", "CRITICAL", "HIGH", "MEDIUM"], index=0)
    ...
    with col2:
        ...
        if st.button("Export Alert Report", use_container_width=True):
            if alerts:
                ...
                st.download_button("Download CSV", csv, "alert_report.csv", "text/csv", key="download_alerts")
