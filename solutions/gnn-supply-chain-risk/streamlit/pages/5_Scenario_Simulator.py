...
from utils.data_loader import DB_SCHEMA
...
st.set_page_config(page_title="Scenario Simulator", page_icon=None, layout="wide", initial_sidebar_state="expanded")
...
def render_impact_network(affected_vendors, affected_materials, disruption_source, height=450):
    ...
    graph.add_node(disruption_source, node_type="disruption", level=0)

    # Add affected vendors
    for _, row in affected_vendors.iterrows():
        vendor_id = row["VENDOR_ID"]
        vendor_label = row.get("NAME") or row.get("VENDOR_NAME") or vendor_id
        graph.add_node(
            vendor_id,
            node_type="vendor",
            label=vendor_label,
        ...
        graph.add_edge(disruption_source, vendor_id)

    # Add affected materials
    for _, row in affected_materials.iterrows():
        mat_id = row["MATERIAL_ID"]
        if mat_id not in graph.nodes():
            graph.add_node(mat_id, node_type="material", label=row.get("DESCRIPTION", mat_id)[:25], level=2)
        # Connect to vendor if we have that info
        if "VENDOR_ID" in row and row["VENDOR_ID"] in graph.nodes():
            graph.add_edge(row["VENDOR_ID"], mat_id)
    ...
    edge_trace = go.Scatter(x=edge_x, y=edge_y, mode="lines", line=dict(width=2, color="#dc2626"), hoverinfo="none")
    ...
    for node_type, config in type_config.items():
        ...
        trace = go.Scatter(
            ...
            mode="markers",
            marker=dict(
                size=config["size"], color=config["color"], symbol=config["symbol"], line=dict(width=2, color="#0f172a")
            ),
            text=node_text,
            hoverinfo="text",
            name=config["name"],
...
def main():
        ...
        if scenario_type == "Regional Disruption":
            if not regions.empty:
                ...
                if has_critical_narrative(selected_region):
                    ...
                    st.markdown(
                        render_risk_intelligence_card(selected_region, show_bottleneck=True), unsafe_allow_html=True
                    )
                else:
                    ...
                    <div style="background: rgba(30, 41, 59, 0.6);
                        ...
                        <div style="color: #94a3b8; font-size: 0.85rem;">
                            <div>Geopolitical Risk: <strong
                                style="color: #f8fafc;">{region_info["GEOPOLITICAL_RISK"]:.0%}</strong></div>
                            <div>Natural Disaster Risk: <strong
                                style="color: #f8fafc;">{region_info["NATURAL_DISASTER_RISK"]:.0%}</strong></div>
                            <div>Vendors in Region: <strong
                                style="color: #f8fafc;">{region_info["VENDOR_COUNT"]}</strong></div>
                        </div>
                    </div>
                    """,
                        unsafe_allow_html=True,
            ...
            if not bottlenecks.empty:
                ...
                <div style="background: rgba(30, 41, 59, 0.6); border-radius: 8px; padding: 1rem; margin-top: 1rem;">
                    <div style="color: #94a3b8; font-size: 0.85rem;">
                        <div>Impact Score: <strong
                            style="color: #dc2626;">{bottleneck_info["IMPACT_SCORE"]:.0%}</strong></div>
                        <div>Dependent Vendors: <strong
                            style="color: #f8fafc;">{bottleneck_info["DEPENDENT_COUNT"]}</strong></div>
                        <div style="margin-top: 0.5rem; color: #64748b;
                            font-size: 0.8rem;">{
                        bottleneck_info.get("DESCRIPTION", "Hidden supply chain convergence point")
                    }</div>
                    </div>
                </div>
                """,
    ...
    if not affected_vendors.empty or not affected_materials.empty:
        ...
        with col3:
            ...
            high_crit = (
                len(affected_materials[affected_materials["CRITICALITY_SCORE"].fillna(0) >= 0.7])
        ...
        <div class="scenario-result">
            ...
            <p style="color: #e2e8f0; line-height: 1.6;">
                ...
                {
                f"This could cascade to impact <strong>{downstream_count} finished product(s)</strong>."
                if downstream_count > 0
                else ""
            }
                {
                f"We identified <strong>"
                f"{len(alternatives) if 'alternatives' in dir() and not alternatives.empty else 0}"
                f" alternative supplier(s)</strong> that could help mitigate this risk."
                if not affected_materials.empty
                else ""
            }
            </p>
