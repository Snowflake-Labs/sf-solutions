...
st.set_page_config(page_title="Exploratory Analysis", page_icon=None, layout="wide", initial_sidebar_state="expanded")
...
def render_data_flow_sankey(stats, height=350):
    ...
    # Node indices: 0=ERP, 1=Vendors, 2=Materials, 3=POs, 4=BOM,
    # 5=Trade Src, 6=Trade, 7=Graph, 8=GNN, 9=Predictions, 10=Risks

    # Category colors
    colors = {"ERP": "#3b82f6", "TRADE": "#f59e0b", "GRAPH": "#8b5cf6", "INFERENCE": "#10b981"}
    ...
    node_colors = [
        ...
        colors["ERP"],
        colors["ERP"],
        colors["ERP"],  # ERP nodes
        colors["TRADE"],
        colors["TRADE"],  # Trade nodes
        colors["GRAPH"],  # Graph node
        colors["INFERENCE"],
        colors["INFERENCE"],
        colors["INFERENCE"],  # Inference nodes
    ]
    ...
    link_colors = [
        ...
        "rgba(59, 130, 246, 0.4)",
        "rgba(59, 130, 246, 0.4)",
        "rgba(59, 130, 246, 0.4)",  # ERP to children
        "rgba(59, 130, 246, 0.4)",
        "rgba(59, 130, 246, 0.4)",
        "rgba(59, 130, 246, 0.4)",
        "rgba(59, 130, 246, 0.4)",  # Children to graph
        "rgba(245, 158, 11, 0.4)",
        "rgba(245, 158, 11, 0.4)",  # Trade flow
        "rgba(139, 92, 246, 0.4)",  # Graph to GNN
        "rgba(16, 185, 129, 0.4)",
        "rgba(16, 185, 129, 0.4)",  # GNN outputs
    ]
...
def main():
    ...
    with col2:
        st.markdown(
            '<div style="text-align: center; padding-top: 1.5rem; color: #64748b; font-size: 2rem;">→</div>',
            unsafe_allow_html=True,
        )
        ...
        <div style="text-align: center; padding: 1rem;">
            <div style="background: #166534; border: 2px solid #22c55e; border-radius: 8px; padding: 1rem;">
                ...
                <div style="color: #86efac; font-size: 0.9rem;">
                    {stats.get("VENDORS", {}).get("count", 0)} Direct Suppliers</div>
    ...
    with col1:
        ...
        if trade_flows is not None and not trade_flows.empty:
            ...
            for _, row in trade_flows.head(5).iterrows():
                st.markdown(
                    f"- **{row['SHIPPER_COUNTRY']}**: "
                    f"{row['SHIPMENT_COUNT']} shipments, "
                    f"{row['SHIPPER_COUNT']} unique shippers"
                )
    ...
    with col2:
        ...
        if geo_dist is not None and not geo_dist.empty:
            ...
            for _, row in geo_dist.head(5).iterrows():
                risk_label = (
                    "High"
                    if row.get("REGION_RISK", 0) > 0.5
                    else "Medium"
                    if row.get("REGION_RISK", 0) > 0.25
                    else "Low"
                )
                st.markdown(
                    f"- **{row['COUNTRY_CODE']}** "
                    f"({row.get('REGION_NAME', 'Unknown')}): "
                    f"{row['VENDOR_COUNT']} vendors [{risk_label} risk]"
                )
        ...
        <div class="data-card" style="border-color: #10b981;">
            <h3>Predicted Links
                <span style="background: #166534; color: #fff;
                    ...
                    font-size: 0.7rem;">GNN OUTPUT</span></h3>
            <div class="data-count" style="color: #10b981;">
                {pl.get("count", 0):,}</div>
            <p>{pl.get("description", "")}
                — Hidden Tier-2+ relationships that the GNN
                discovered by analyzing trade patterns</p>
        </div>
        """,
            unsafe_allow_html=True,
        )
    ...
    with col2:
        ...
        <div class="data-card" style="border-color: #10b981;">
            ...
            <div class="data-count" style="color: #10b981;">
                {rs.get("count", 0):,}</div>
            <p>{rs.get("description", "")}
                — Every node in the graph now has a propagated risk
                score based on its position in the network</p>
        </div>
        """,
            unsafe_allow_html=True,
        )
