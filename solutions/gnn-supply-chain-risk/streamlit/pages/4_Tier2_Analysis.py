...
st.set_page_config(page_title="Tier-2 Analysis", page_icon=None, layout="wide", initial_sidebar_state="expanded")
...
def prefetch_all_ai_analyses(_session, bottleneck_data: list):
    ...
    def generate_single_analysis(data: dict) -> tuple:
        ...
        try:
            prompt = (
                ...
                "Provide a brief risk assessment for "
                "this concentration point in 2-3 sentences.\n\n"
                f"Bottleneck: {node_id}\n"
                f"- Dependent Vendors: {data.get('dependent_count', 0)}\n"
                f"- Impact Score: {data.get('impact_score', 0):.0%}\n"
                f"- Mitigation Status: {data.get('mitigation_status', 'UNMITIGATED')}\n\n"
                "Explain the specific risk this creates and one key "
                "mitigation action.\n\n"
                "Output only the analysis. No preamble, headers, "
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(generate_single_analysis, data): data["node_id"] for data in bottleneck_data}
...
def generate_ai_analysis(_session, context_type: str, context_data: dict):
    ...
    try:
        if context_type == "concentration_risk":
            prompt = (
                "You are a supply chain risk analyst. "
                "Analyze this concentration risk finding and explain "
                "its business implications in 2-3 concise sentences.\n\n"
                "Context:\n"
                f"- Hidden Tier-2 Supplier: {context_data.get('node_id', 'Unknown')}\n"
                f"- Number of your Tier-1 vendors dependent on this supplier: "
                f"{context_data.get('dependent_count', 0)}\n"
                f"- Impact Score: {context_data.get('impact_score', 0):.0%}\n"
                f"- Description: {context_data.get('description', 'Supply chain convergence point')}\n\n"
                "Explain why this is a risk and what could happen if this supplier "
                "experiences disruption. Be specific and actionable.\n\n"
            ...
            prompt = (
                ...
                "Provide a brief risk assessment for "
                "this concentration point in 2-3 sentences.\n\n"
                f"Bottleneck: {context_data.get('node_id', 'Unknown')}\n"
                f"- Dependent Vendors: {context_data.get('dependent_count', 0)}\n"
                f"- Impact Score: {context_data.get('impact_score', 0):.0%}\n"
                f"- Mitigation Status: {context_data.get('mitigation_status', 'UNMITIGATED')}\n\n"
...
def render_ego_graph(center_id, center_label, dependents, height=400):
    ...
    for i, (_, row) in enumerate(dependents.iterrows()):
        ...
        dep_text.append(
            f"<b>{name}</b><br>Country: {row['COUNTRY_CODE']}<br>Confidence: {prob:.0%}<br>Risk: {risk:.0%}"
        )
    ...
    edge_trace = go.Scatter(
        x=edge_x, y=edge_y, mode="lines", line=dict(width=2, color="#f59e0b", dash="dash"), hoverinfo="none"
    )
    ...
    label_trace = go.Scatter(
        x=dep_x, y=label_y, mode="text", text=labels, textfont=dict(size=9, color="#cbd5e1"), hoverinfo="none"
    )
...
def main():
    """Render the Tier-2 Analysis page."""
    session = get_session()
    ...
    if bottlenecks is not None and not bottlenecks.empty:
        ...
        if region_code:
            ...
            st.markdown(render_risk_intelligence_card(region_code, show_bottleneck=False), unsafe_allow_html=True)
        ...
        with col1:
            ...
            if dependents is not None and not dependents.empty:
                render_ego_graph(top_bottleneck["NODE_ID"], top_bottleneck["NODE_ID"], dependents, height=380)
            else:
                st.info("No dependency data available.")
        ...
        with col2:
            ...
            if dependents is not None and not dependents.empty:
                ...
                dep_df["COLOR"] = dep_df["RISK"].apply(
                    lambda x: "#dc2626" if x >= 0.5 else "#f59e0b" if x >= 0.25 else "#16a34a"
                )
        ...
        for idx, row in bottlenecks.iterrows():
            ...
            <div class="bottleneck-card {impact_class}">
                ...
                <div class="evidence-section">
                    ...
                    {
                    f'<span style="color: #64748b; font-size: 0.8rem;"> +{len(deps) - 5} more</span>'
                    if deps is not None and len(deps) > 5
                    else ""
                }
                </div>
            </div>
            ...
            if bottleneck_analysis:
                ...
                <div style="background: rgba(59, 130, 246, 0.08);
                    ...
                    <div style="color: #60a5fa;
                        ...
                        text-transform: uppercase;
                        letter-spacing: 0.05em;
                        margin-bottom: 0.25rem;">AI Analysis</div>
                    <p style="color: #e2e8f0;
                        line-height: 1.5; margin: 0;
                        font-size: 0.9rem;">{bottleneck_analysis}</p>
                </div>
                """,
                    unsafe_allow_html=True,
                )
            ...
            if bottleneck_region:
                st.markdown(render_compact_risk_card(bottleneck_region), unsafe_allow_html=True)
    else:
    ...
    if predicted_links is not None and not predicted_links.empty:
        ...
        with st.spinner("Generating AI summary..."):
            links_summary = generate_ai_analysis(
                session,
                "links_summary",
                {"total_links": len(predicted_links), "high_confidence": high_conf_count, "avg_confidence": avg_conf},
            )
        ...
        styled_df = (
            predicted_links[display_cols]
            .style.applymap(style_probability, subset=["PROBABILITY"])
        ...
        st.download_button("Download Predicted Links CSV", csv, "predicted_links.csv", "text/csv")
    else:
