...
st.set_page_config(page_title="Executive Summary", page_icon=None, layout="wide", initial_sidebar_state="expanded")
    ...
    fig = go.Figure(
        go.Indicator(
            ...
            gauge={
                ...
                "threshold": {"line": {"color": "#f8fafc", "width": 2}, "thickness": 0.8, "value": score},
            },
        )
    )
...
def render_regional_heatmap(regional_data, height=350):
    ...
    colors = df["AVG_RISK"].apply(lambda x: "#dc2626" if x >= 0.6 else "#f59e0b" if x >= 0.4 else "#10b981").tolist()
    ...
    fig.update_layout(
        ...
        yaxis=dict(title=None, tickfont=dict(color="#e2e8f0", size=11)),
        bargap=0.3,
    )
...
def main():
    ...
    with col1:
        ...
        # Status badge
        status_color = (
            "#10b981" if health_status == "Healthy" else "#f59e0b" if health_status == "Moderate" else "#dc2626"
        )
        st.markdown(
            f"""
    ...
    with col2:
        ...
        with kpi_col2:
            bottleneck_count = (
                int(bottleneck_summary["TOTAL_BOTTLENECKS"].iloc[0] or 0)
                if bottleneck_summary is not None and not bottleneck_summary.empty
                else 0
            )
            st.markdown(
        ...
        with kpi_col3:
            total_vendors = (
                int(vendor_summary["TOTAL_VENDORS"].iloc[0] or 0)
        ...
        with kpi_col4:
            predicted_links = (
                int(prediction_summary["TOTAL_PREDICTIONS"].iloc[0] or 0)
                if prediction_summary is not None and not prediction_summary.empty
                else 0
            )
            st.markdown(
                f"""
    ...
    with col3:
        ...
        if risk_summary is not None and not risk_summary.empty:
            ...
            <div class="insight-card">
                ...
                {
                    "This is elevated and requires attention." if avg_risk > 0.5 else "This is within acceptable range."
                }</p>
            </div>
            """,
                unsafe_allow_html=True,
    ...
    with calc_col1:
        ...
        hourly_rate = st.number_input(
            "Average analyst hourly rate ($)", min_value=25, max_value=500, value=150, step=25
        )
    ...
    with calc_col2:
        ...
        <div style="background: rgba(16, 185, 129, 0.1);
            ...
            <div style="color: #10b981; font-size: 0.85rem;
                text-transform: uppercase;
                margin-bottom: 0.5rem;">Total Estimated Annual Value</div>
