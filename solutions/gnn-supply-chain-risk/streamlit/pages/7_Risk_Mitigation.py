...
from utils.data_loader import DB_SCHEMA, run_queries_parallel
from utils.sidebar import render_sidebar, render_star_callout

st.set_page_config(page_title="Risk Mitigation", page_icon=None, layout="wide", initial_sidebar_state="expanded")
    ...
    actions.append(
        {
            ...
            "description": (
                "Refresh external trade intelligence data to ensure predictions reflect current market conditions."
            ),
            "impact_score": 0.3,
            "action_type": "routine",
        }
...
def _build_action_prompt(action: dict) -> str:
    ...
    if action_type == "bottleneck":
        return (
            "You are a supply chain risk advisor. Provide a detailed "
            "explanation and specific mitigation strategies for this "
            "recommended action.\n\n"
            f"Action: {action['title']}\n"
            "Context:\n"
            f"- This is a hidden Tier-2 supplier that "
            f"{action.get('dependent_count', 0)} of our Tier-1 vendors depend on\n"
            f"- Impact score: {action.get('impact_score', 0):.0%}\n\n"
            "Provide 3-4 specific, actionable mitigation strategies. "
            "Be concise but practical. Include:\n"
            "1. Immediate actions (within 30 days)\n"
            "2. Medium-term actions (1-3 months)\n"
            "3. Long-term strategic changes\n\n"
            "Format as a brief paragraph for each strategy.\n\n"
            "Output only the strategies. No preamble, headers, "
            "or follow-up questions."
        )

    elif action_type == "high_risk_supplier":
        return (
            "You are a supply chain risk advisor. Provide detailed "
            "guidance for managing this high-risk supplier situation.\n\n"
            f"Action: {action['title']}\n"
            "Context:\n"
            f"- Supplier: {action.get('supplier_name', 'Unknown')}\n"
            f"- Location: {action.get('country', 'Unknown')}\n"
            f"- Risk Score: {action.get('risk_score', 0):.0%}\n\n"
            "Provide 3-4 specific recommendations for risk mitigation. "
            "Consider:\n"
            ...
            "Output only the recommendations. No preamble, headers, "
            "or follow-up questions."
        )

    else:
        return (
            "You are a supply chain operations advisor. Provide practical "
            "implementation guidance for this recommended action.\n\n"
            f"Action: {action['title']}\n"
            f"Description: {action.get('description', '')}\n\n"
            "Provide 2-3 specific implementation steps with timelines "
            "and key considerations. Be concise and actionable.\n\n"
            "Output only the steps. No preamble, headers, "
            "or follow-up questions."
        )
...
def prefetch_all_action_explanations(_session, actions: list):
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(generate_single_explanation, idx, action): idx for idx, action in enumerate(actions)}
...
def render_risk_matrix(df, height=400):
    ...
    color_scale = alt.Scale(
        domain=["Critical", "Manage", "Monitor", "Accept"], range=["#dc2626", "#ea580c", "#ca8a04", "#16a34a"]
    )
    ...
    quadrant_labels = (
        alt.Chart(quadrants)
        .mark_text(fontSize=11, fontWeight="bold", opacity=0.5)
        .encode(x=alt.X("label_x:Q"), y=alt.Y("label_y:Q"), text="label:N")
        .transform_calculate(label_x="(datum.x + datum.x2) / 2", label_y="(datum.y + datum.y2) / 2")
    )

    # Threshold lines
    vline = alt.Chart(pd.DataFrame({"x": [0.5]})).mark_rule(strokeDash=[4, 4], stroke="#475569").encode(x="x:Q")

    hline = alt.Chart(pd.DataFrame({"y": [0.5]})).mark_rule(strokeDash=[4, 4], stroke="#475569").encode(y="y:Q")

    # Main scatter plot
    points = (
        alt.Chart(df)
        .mark_circle(size=120, stroke="#0f172a", strokeWidth=1.5)
        .encode(
            x=alt.X(
                "IMPACT:Q", scale=alt.Scale(domain=[0, 1]), axis=alt.Axis(title="Impact Score", format="%", tickCount=5)
            ),
            y=alt.Y(
                "PROBABILITY:Q",
            ...
            color=alt.Color("QUADRANT:N", scale=color_scale, legend=alt.Legend(title="Risk Level")),
            tooltip=[
    ...
    # Combine all layers
    chart = (
        (quadrant_bg + vline + hline + quadrant_labels + points)
        .properties(height=height)
        .configure_axis(labelColor="#94a3b8", titleColor="#e2e8f0", gridColor="#334155", domainColor="#475569")
...
def main():
            ...
            if ai_explanation:
                ...
                <div style="background: rgba(59, 130, 246, 0.08);
                    ...
                    <div style="color: #60a5fa;
                        ...
                        margin-bottom: 0.25rem;">AI Guidance</div>
                    <p style="color: #e2e8f0; line-height: 1.6;
                        white-space: pre-wrap; margin: 0;
                        font-size: 0.9rem;">{ai_explanation}</p>
                </div>
                """,
                    unsafe_allow_html=True,
                )
    ...
    with col2:
        ...
        if high_risk is not None and not high_risk.empty:
            for _, row in high_risk.head(8).iterrows():
                risk = row["RISK_SCORE"]
                risk_class = (
                    "critical" if risk >= 0.75 else "high" if risk >= 0.5 else "medium" if risk >= 0.25 else "low"
                )
                ...
                with st.spinner("Analyzing..."):
                    ...
                    if result is not None and not result.empty:
                        ...
                        st.session_state.messages.append(
                            {"role": "assistant", "content": "Here's what I found:", "data": result}
                        )
                    elif result is not None:
                        st.markdown("The query returned no results.")
                        st.session_state.messages.append(
                            {"role": "assistant", "content": "The query returned no results."}
                        )
                    else:
                        st.markdown(f"I couldn't process that. {info}")
                        st.session_state.messages.append(
                            {"role": "assistant", "content": f"I couldn't process that. {info}"}
                        )
        ...
        for q in SAMPLE_QUESTIONS:
            if st.button(q, key=f"q_{q[:20]}", use_container_width=True):
                ...
                if result is not None:
                    ...
                    st.session_state.messages.append({"role": "assistant", "content": f"Error: {info}"})
                st.rerun()
    ...
    with col1:
        if high_risk is not None:
            ...
            st.download_button(
                "Download Risk Report (CSV)", csv, "risk_report.csv", "text/csv", use_container_width=True
            )
    ...
    with col2:
        try:
            ...
            if not bottlenecks.empty:
                ...
                st.download_button(
                    "Download Bottleneck Report", csv, "bottleneck_report.csv", "text/csv", use_container_width=True
                )
        except Exception:
    ...
    with col3:
        try:
            links = session.sql(
                "SELECT * FROM PREDICTED_LINKS WHERE PROBABILITY >= 0.5 ORDER BY PROBABILITY DESC"
            ).to_pandas()
            if not links.empty:
                csv = links.to_csv(index=False)
                st.download_button(
                    "Download Predictions", csv, "predicted_links.csv", "text/csv", use_container_width=True
                )
        except Exception:
