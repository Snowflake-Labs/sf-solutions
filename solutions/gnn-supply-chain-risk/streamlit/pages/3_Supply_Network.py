...
st.set_page_config(page_title="Supply Network", page_icon=None, layout="wide", initial_sidebar_state="expanded")
...
def load_graph_data(_session, include_predicted=True):
    ...
    try:
        ...
        for _, row in pos.iterrows():
            edges.append({"source": row["SOURCE"], "target": row["TARGET"], "type": "SUPPLIES", "predicted": False})
    except Exception:
        pass
        ...
        try:
            ...
            for ext_id in ext_suppliers:
                if not any(n["id"] == ext_id for n in nodes):
                    nodes.append({"id": ext_id, "label": ext_id, "type": "EXTERNAL_SUPPLIER", "risk_score": 0.6})
    ...
    try:
        ...
        for root_id in list(roots)[:3]:
            ...
            if not root_rows.empty:
                ...
                trees.append(
                    build_tree(root_id, root_info["PARENT_DESC"] or root_id, root_info["PARENT_GROUP"] or "FIN")
                )
...
def render_full_graph(nodes, edges, height=600):
    ...
    # Color maps
    type_colors = {"SUPPLIER": "#3b82f6", "MATERIAL": "#8b5cf6", "EXTERNAL_SUPPLIER": "#f59e0b"}
...
def render_bom_tree(tree_data, height=500):
    ...
    type_colors = {"FIN": "#10b981", "SEMI": "#8b5cf6", "RAW": "#f59e0b"}

    type_labels = {"FIN": "Finished Good", "SEMI": "Semi-finished", "RAW": "Raw Material"}
    ...
    fig = go.Figure(
        go.Treemap(
            ...
            parents=parents,
            values=values,
            marker=dict(colors=colors, line=dict(width=2, color="#0f172a"), cornerradius=5),
            branchvalues="total",
            ...
            maxdepth=3,
            pathbar=dict(visible=True, thickness=28, textfont=dict(size=12, color="#e2e8f0"), edgeshape=">"),
        )
    )
    ...
    fig.update_layout(
        ...
        font=dict(color="#e2e8f0"),
        hoverlabel=dict(
            bgcolor="#1e293b", bordercolor="#334155", font=dict(size=12, color="#e2e8f0", family="system-ui")
        ),
    )
...
def main():
    """Render the Supply Network page."""
    session = get_session()
    ...
    try:
        ...
        if not region_data.empty:
            ...
            region_data["DISPLAY_NAME"] = region_data.apply(
                lambda r: f"{r['REGION_NAME']} ({r['COUNTRY_CODE']})" if r["REGION_NAME"] else r["COUNTRY_CODE"], axis=1
            )
            ...
            if not map_data.empty:
                ...
                view_state = pdk.ViewState(
                    latitude=20, longitude=0, zoom=0.9, pitch=45, bearing=0, min_zoom=0.5, max_zoom=6
                )
                ...
                deck = pdk.Deck(
                    ...
                    tooltip={
                        "html": "<b>{COUNTRY_NAME}</b><br>Vendors: {VENDOR_COUNT}<br>Risk: {RISK_DISPLAY}",
                        "style": {
